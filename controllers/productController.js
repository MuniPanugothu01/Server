...
// Configure Cloudinary (put your real creds or load from env)
cloudinary.config({
...
const addProduct = async (req, res) => {
  try {
    console.log("Product Add Request Body:", req.body);
    const { name, description, category, price, offerPrice } = JSON.parse(
      req.body.productData
    );
    ...
    const product = new Product({
      ...
      price,
      offerPrice,
      image: imagesUrl[0], // store only the first image for now
      category,
    });
